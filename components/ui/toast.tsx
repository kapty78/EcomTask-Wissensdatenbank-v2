...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      "fixed top-2 right-2 z-[100] flex max-h-screen w-full sm:w-auto flex-col gap-2 p-0 md:max-w-[420px]",
      className
    )}
    {...props}
  />
...
const toastVariants = cva(
  "data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none",
  {
    variants: {
      variant: {
        default: "bg-[#242424] text-white border-[#333333]",
        destructive:
          "destructive border-[#333333] bg-[#242424] text-white group"
      }
    },
    defaultVariants: {
      variant: "default"
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "text-foreground/50 hover:text-foreground absolute right-2 top-2 rounded-md p-1 opacity-0 transition-opacity focus:opacity-100 focus:outline-none focus:ring-2 group-hover:opacity-100 group-[.destructive]:text-gray-300 group-[.destructive]:hover:text-white group-[.destructive]:focus:ring-gray-400 group-[.destructive]:focus:ring-offset-gray-600",
      className
    )}
    toast-close=""
    {...props}
