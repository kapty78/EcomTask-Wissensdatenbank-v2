...
-- Datum: 2025-01-15
-- Zweck: Fehlende Spalten für Chunk-Processing hinzufügen

-- Überprüfe, ob die Tabelle existiert und füge fehlende Spalten hinzu
DO $$
BEGIN
    IF EXISTS (
        ...
        -- KRITISCHE SPALTE: Chunk-Verarbeitungszeit (für Performance-Monitoring)
        IF NOT EXISTS (
            SELECT FROM information_schema.columns 
            WHERE table_schema = 'public' 
            AND table_name = 'document_chunks' 
...
-- RPC-Funktion für erweiterte Chunk-Statistiken hinzufügen
